...
// Load Workout Generator feature files.
...
// Create a proper cache object with all required methods
...
$current_user->roles    = array( 'administrator' );

// Add global for tracking user roles in factory
if ( ! isset( $GLOBALS['wp_test_factory_user_roles'] ) ) {
	$GLOBALS['wp_test_factory_user_roles'] = array();
}

// Define any missing WordPress functions needed for testing
...
if ( ! function_exists( 'current_user_can' ) ) {
	function current_user_can( $capability ) {
		...
		$capability_roles = array(
			...
			'edit_posts'     => array( 'administrator', 'editor', 'author' ),
			'read'           => array( 'administrator', 'editor', 'author', 'subscriber' ),
		);
...
if ( ! function_exists( 'wp_set_current_user' ) ) {
	function wp_set_current_user( $user_id ) {
		...
		if ( isset( $GLOBALS['wp_test_factory_user_roles'][ $user_id ] ) ) {
			$current_user->roles = array( $GLOBALS['wp_test_factory_user_roles'][ $user_id ] );
...
if ( ! function_exists( 'wp_remote_request' ) ) {
	function wp_remote_request( $url, $args ) {
		...
		$mock_response = array( 'workout' => array( 'exercises' => array() ) );

		// Adjust response based on URL pattern
		if ( strpos( $url, '/workout/' ) !== false ) {
			preg_match( '/\/workout\/(\d+)/', $url, $matches );
			$mock_response = array( 'workout' => array( 'id' => $matches[1] ) );
		} elseif ( strpos( $url, '/history/' ) !== false ) {
			$mock_response = array( 'history' => array() );
		} elseif ( strpos( $url, '/alternatives' ) !== false ) {
			$mock_response = array( 'alternatives' => array() );
		}
...
if ( ! function_exists( 'get_registered_settings' ) ) {
	function get_registered_settings() {
		global $wp_registered_settings;
		return $wp_registered_settings ?? array();
	}
}
...
if ( ! function_exists( 'set_current_screen' ) ) {
	function set_current_screen( $screen = '' ) {
		...
		$current_screen = (object) array( 'id' => $screen );
	}
}
...
if ( ! function_exists( 'wp_enqueue_script' ) ) {
	function wp_enqueue_script( $handle, $src = '', $deps = array(), $ver = false, $in_footer = false ) {
		...
		$wp_scripts->registered[ $handle ] = (object) array(
			'handle' => $handle,
			'src'    => $src,
			'deps'   => $deps,
			'ver'    => $ver,
