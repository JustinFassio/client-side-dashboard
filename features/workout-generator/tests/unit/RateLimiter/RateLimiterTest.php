...
class RateLimiterTest extends TestCase {
	...
	public function test_multiple_requests_within_limit() {
		...
		// Set foundation tier
		add_filter(
			'athlete_dashboard_get_user_tier',
			function () {
				return 'foundation';
			}
		);
	...
	public function test_limit_reset_after_window() {
		...
		global $wp_transients;
		$wp_transients = array(); // Clear transients to simulate expiration
	...
	private function simulate_time_passing( $seconds ) {
		global $wp_transients;
		$wp_transients = array(); // Clear all transients to simulate expiration
	}
	...
	public function test_tier_rate_limit_headers() {
		...
		$this->assertArrayHasKey( 'X-RateLimit-Limit', $headers, 'Headers should include rate limit' );
		$this->assertArrayHasKey( 'X-RateLimit-Remaining', $headers, 'Headers should include remaining requests' );
		$this->assertArrayHasKey( 'X-RateLimit-Reset', $headers, 'Headers should include reset time' );

		$this->assertEquals( 60, $headers['X-RateLimit-Limit'], 'Foundation tier should have 60 requests limit' );
		$this->assertEquals( 60, $headers['X-RateLimit-Remaining'], 'Should start with full limit' );
